...
";
  ...
  public static function initialize() {
    ...
    if (!FacebookPluginUtils::is_positive_integer($pixel_id)) {
      $fb_warm_pixel_id = FacebookPixelConfigWarmer::$fb_warm_pixel_id;

      if (FacebookPluginUtils::is_positive_integer($fb_warm_pixel_id) &&
...
<!-- %s Facebook Integration end -->
    ",
    FacebookPluginUtils::INTEGRATION_NAME,
    self::get_basecode(),
   ...
   * instead of FacebookPixel::build_event()
   */
...
<!-- End Facebook Pixel Code -->
  ...
  /**
   * You probably should use FacebookPixel::inject_event() but
  ...
  private static function get_version_info() {
    ...
    return array(
      'source' => 'wordpress',
      'version' => $wp_version,
      'pluginVersion' => FacebookPixelPlugin::PLUGIN_VERSION
    );
  }
