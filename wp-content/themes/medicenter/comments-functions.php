...

{
...

	{
...

		{
			...
			$result["submit_message"] = __("Votre message est posté !", 'medicenter');
...

	{
		...
		if($_POST["email"]=="" || $_POST["email"]==__("Votre email", 'medicenter') || !preg_match("#^[_a-zA-Z0-9-]+(\.[_a-zA-Z0-9-]+)*@[a-zA-Z0-9-]+(\.[a-zA-Z0-9-]+)*(\.[a-zA-Z]{2,4})$#", $_POST["email"]))

			$result["error_email"] = __("Merci de saisir une adresse valide", 'medicenter');

		if($_POST["message"]=="" || $_POST["message"]==__("Message", 'medicenter'))

			$result["error_message"] = __("Veuillez saisir votre message", 'medicenter');
