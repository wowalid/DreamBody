	...
	?>
		...
		<div class="comment_box<?php echo ($comments_form_animation!='' ? ' animated_element animation-' . $comments_form_animation . ((int)$comments_form_animation_duration>0 && (int)$comments_form_animation_duration!=600 ? ' duration-' . (int)$comments_form_animation_duration : '') . ((int)$comments_form_animation_delay>0 ? ' delay-' . (int)$comments_form_animation_delay : '') : ''); ?>">
			...
			<div class="comments_number">
				...
				<a href="#comments_list" title="<?php echo $comments_count = get_comments_number(); echo " " . ($comments_count!=1 ? __("commentaires", 'medicenter') : __("comentaires", 'medicenter')); ?>"><?php echo $comments_count = get_comments_number(); echo " " . ($comments_count!=1 ? __("commentaires", 'medicenter') : __("commentaire", 'medicenter')); ?></a>
	...
	<li <?php comment_class('comment clearfix'); ?> id="comment-<?php comment_ID() ?>">
		...
		<div class="comment_details">
			...
			<div class="posted_by">
				...
				_e("Posté par ", 'medicenter'); comment_author_link();

				_e(" le", 'medicenter'); 

				printf(__(' %1$s, %2$s'), get_comment_date(),  get_comment_time());

				edit_comment_link(__('(Editer)', 'medicenter'),'&nbsp;&nbsp;&nbsp;','');

				if((int)$comment->comment_parent>0)

				{

					echo '<br /><a class="show_source_comment" href="#comment-' . (int)$comment->comment_parent . '" title="' . __('Voir commentaires', 'medicenter') . '">';

					_e('en réponse à ', 'medicenter');
			...
			<a class="icon_small_arrow right_white reply_button" href="#<?php comment_ID(); ?>" title="<?php _e('Répondre', 'medicenter'); ?>">

				<?php _e('Répondre &rarr;', 'medicenter'); ?>
