...

{
	...
	$output .= '<fieldset class="left">
		...
		<label>' . __("Prénom", 'medicenter') . '</label>
			...
			<label>' . __("Téléphone", 'medicenter') . '</label>
		...
		<fieldset style="clear:both;">
			...
			<label>' . __("Sujet", 'medicenter') . '</label>
			...
			<input type="submit" name="submit" value="' . __("Envoyer", 'medicenter') . '" class="more mc_button" />
...

{
...

	{
		...
		if($mail->Send())
			...
			$result["submit_message"] = __("Merci pour votre message !", 'medicenter');
...

		{
			...
			$result["submit_message"] = __("Oups, votre message n'a pas été envoyé.", 'medicenter');
...

	{
		...
		if($_POST["first_name"]=="")
			...
			$result["error_first_name"] = __("Merci d'entrer votre prénom.", 'medicenter');

		if($_POST["last_name"]=="")

			$result["error_last_name"] = __("Merci d'entrer votre nom", 'medicenter');

		if($_POST["email"]=="" || !preg_match("#^[_a-zA-Z0-9-]+(\.[_a-zA-Z0-9-]+)*@[a-zA-Z0-9-]+(\.[a-zA-Z0-9-]+)*(\.[a-zA-Z]{2,4})$#", $_POST["email"]))

			$result["error_email"] = __("Merci d'entrer un mail valide.", 'medicenter');

		if($_POST["message"]=="")

			$result["error_message"] = __("Merci de saisir votre message.", 'medicenter');
